...
test.describe("Search page", () => {
  test("shows empty state when no query is provided", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "Search Recipes" })).toBeVisible();
  ...
  test("search bar has autofocus on search page", async ({ page }) => {
    ...
    const searchInput = page.getByRole("searchbox", { name: "Search for recipes..." });
  ...
  test("typing a query and submitting navigates with query param", async ({ page }) => {
    await page.goto("/search");
    ...
    await expect(page.getByText('Showing results for "breakfast"')).toBeVisible();
  });
});
