...
railway run --service surprising-growth npm run db:migrate  # Run with specific service
```

**Required Environment Variables:**

- `DATABASE_URL` - PostgreSQL connection string (access via Railway)
- `OPENAI_API_KEY` - For embeddings and AI recipe parsing (optional but recommended)
...
```bash
railway run --service surprising-growth npm run db:studio    # Run Prisma Studio with Railway DB
railway run --service surprising-growth npm run db:migrate   # Run migrations against Railway DB
```
