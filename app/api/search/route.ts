...
export async function POST(request: NextRequest) {
  try {
    ...
    const cuisines = new Set(results.map((r) => r.recipe.cuisine).filter(Boolean));
    if (cuisines.size > 0 && cuisines.size <= 3) {
      suggestedFilters.push(`Try filtering by: ${Array.from(cuisines).join(", ")}`);
    }

    const quickRecipes = results.filter((r) => r.recipe.totalTime && r.recipe.totalTime < 30);
